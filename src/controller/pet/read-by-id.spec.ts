...
describe('Read by id Pet', () => {
	...
	it('should be return a pet by id', async () => {
		...
		await request(app.server)
			.get(`/pet/detalhes/${pet.body.pet.id}`)
			.expect(200)
	})

	it('should not be return a pet by id', async () => {
		await request(app.server)
			.get('/pet/detalhes/non-exist')
			.expect(404)
	})
})